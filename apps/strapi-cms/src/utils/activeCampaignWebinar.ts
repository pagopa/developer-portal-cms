...
export const createActiveCampaignList = async (
  event: IWebinarEvent
): Promise<boolean> => {
  if (
...
export const deleteActiveCampaignList = async (
  event: IWebinarEvent
): Promise<boolean> => {
  if (
    !getActiveCampaignIntegrationIsEnabled() ||
  ...
  const webinar = await strapi.db
    .query('api::webinar.webinar')
    .findOne({ where: { documentId: event.params.where.documentId } });
...
export const preventBulkDeletion = (): void => {
  if (getActiveCampaignIntegrationIsEnabled()) {
    throw new errors.ApplicationError(
